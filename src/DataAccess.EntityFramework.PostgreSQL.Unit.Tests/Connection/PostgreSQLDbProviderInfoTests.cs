...
public class PostgreSQLDbProviderInfoTests
{
    [Test]
    public async Task ProviderInfo_is_resolvable_and_has_expected_values()
    {
        ...
        await Assert.That(info!.ProviderKey).IsEqualTo("PostgreSQL");
        await Assert.That(info.ProviderName).Contains("PostgreSQL");
    }
}
