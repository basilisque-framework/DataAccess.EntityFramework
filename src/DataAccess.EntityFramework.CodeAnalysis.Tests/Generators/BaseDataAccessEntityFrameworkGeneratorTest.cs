    ...
    where TVerifier : IncrementalSourceGeneratorVerifier<TGenerator>, new()
{
    ...
    protected virtual TVerifier GetVerifier()
    {
        ...
        var verifier = new TVerifier()
        {
            ...
            TestState =
            {
                AdditionalReferences =
                {
                    ...
                    typeof(Basilisque.DependencyInjection.Registration.DependencyCollection).Assembly
                }
            }
        };
        ...
        //Add a mock for the dependency injection registration extension method 'BAS_DA_EF_DIExt_MAP'.
        //Typically this will be generated by the Basilisque.DependencyInjection source generator, but we don't want to depend on that generator for this test.
        addDependencyInjectionMock(verifier);
    ...
    protected abstract IEnumerable<string> GetExpectedDbContextDependencyInjectionRegistrations();
    ...
    private List<(string Name, string SourceText)> getExpectedSources()
    {
        ...
        addDependencyInjectionExtensionDbContext(result);

        addExpectedMigrationAssemblyProviderSources(result);

        addDependencyInjectionExtensionMigrationAssemblyProvider(result);
    ...
    private static void addDependencyInjectionMock(TVerifier verifier)
    {
        verifier.TestState.Sources.Add($@"
using Basilisque.DependencyInjection.Registration;
using Microsoft.Extensions.DependencyInjection;

namespace Basilisque.DataAccess.EntityFramework.CodeAnalysis.Unit.Tests;
...
{{
    partial void registerExtension_BAS_DA_EF_DIExt_DBC(IServiceCollection services);
    partial void registerExtension_BAS_DA_EF_DIExt_MAP(IServiceCollection services);
}}");
    }

    private void addDependencyInjectionExtensionDbContext(List<(string Name, string SourceText)> result)
    {
        var expectedDbContexts = GetExpectedDbContextDependencyInjectionRegistrations();

        if (!expectedDbContexts.Any())
            return;

        StringBuilder sb = new();

        foreach (var dbContexts in expectedDbContexts)
        {
            sb.AppendLine();
            sb.Append($"        services.AddDbContext<global::{dbContexts}>();");
        }

        result.Add((
            Name: "DependencyRegistrator_RootNamespace.BAS_DA_EF_DIExt_DBC.impl.g.cs",
            SourceText: $@"using Basilisque.DependencyInjection.Registration;
using Microsoft.Extensions.DependencyInjection;
...
{{
    partial void registerExtension_BAS_DA_EF_DIExt_DBC(IServiceCollection services)
    {{
        /* register services for extension 'BAS_DA_EF_DIExt_DBC' */%DBCONTEXT_REGISTRATIONS%
    }}
}}".Replace("%DBCONTEXT_REGISTRATIONS%", sb.ToString())
            ));
    }
    ...
    private static void addDependencyInjectionExtensionMigrationAssemblyProvider(List<(string Name, string SourceText)> result)
    {
        ...
        result.Add((
                    Name: "DependencyRegistrator_RootNamespace.BAS_DA_EF_DIExt_MAP.impl.g.cs",
                    SourceText: $@"using Basilisque.DependencyInjection.Registration;
using Microsoft.Extensions.DependencyInjection;
...
{{
    partial void registerExtension_BAS_DA_EF_DIExt_MAP(IServiceCollection services)
    {{
        /* register services for extension 'BAS_DA_EF_DIExt_MAP' */
        services.AddTransient<global::Basilisque.DataAccess.EntityFramework.Base.Model.IMigrationAssemblyProvider, global::Basilisque.DataAccess.EntityFramework.CodeAnalysis.Unit.Tests.Generated.MigrationAssemblyProvider>();
    }}
}}"
        ));
    }
