...
public class SQLiteDbProviderInfoTests
{
    [Test]
    public async Task ProviderInfo_is_resolvable_and_has_expected_values()
    {
        ...
        await Assert.That(info!.ProviderKey).IsEqualTo("SQLite");
        await Assert.That(info.ProviderName).Contains("Sqlite", System.StringComparison.OrdinalIgnoreCase);
    }
}
